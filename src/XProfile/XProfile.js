...
const XProfile = () => {
    ...
    useEffect(() => {
        async function fetchXUserInfo() {
            try {
                const response = await axios.get(`http://172.16.86.241:8080/${xUserID}/getUserInfo`);
    ...
    const handlePostClick = () => {
        ...
        axios.post(`http://172.16.86.241:8080/chat/${userID}`, { xUserID }).then((response) => {
    ...
    return (
        <div>
            ...
            <div className="op">
                ...
                <div className="사용언어" onClick={() => showModal("language")} >
                    사용언어
                </div>
                <div className="프로젝트이력" onClick={() => showModal("project")}>
                    프로젝트
                </div>
                <div className="관심분야" onClick={() => showModal("field")} >
                    관심분야
                </div>
            </div>
        </div>
