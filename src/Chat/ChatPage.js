...
function ChatPage({ roomId }) {
  ...
  useEffect(() => {
    if (roomId) {
      axios.get(`http://172.16.86.241:8080/api/chat/rooms/${roomId}/messages`)
        .then(response => setMessages(response.data));
    }
  }, [roomId]);

  const sendMessage = () => {
    axios.post(`http://172.16.86.241:8080/api/chat/rooms/${roomId}/messages`, {
      sender: 'user1',
      content: message
    }).then(response => {
...
//   useEffect(() => {
//     if (roomId) {
//       axios.get(`http://172.16.86.241:8080/chat/rooms/${roomId}/messages`)
//         .then(response => setMessages(response.data));
//     }
//   }, [roomId]);

//   const sendMessage = () => {
//     axios.post(`http://172.16.86.241:8080/chat/rooms/${roomId}/messages`, {
//       sender: 'user1',
//       content: message
//     }).then(response => {
//       setMessages([...messages, response.data]);
